...
uniform vec3 l_ball_type;
uniform vec4 l_ball_color;
uniform vec3 l_ball_pos;
uniform float l_ball_target;
uniform float l_ball_decay;
uniform vec3 l_ball_dir;
uniform float l_ball_coneOut;
uniform float l_ball_coneIn;
...
vec4 compLightColor(vec3 l_type, vec3 l_pos, vec4 l_color, float l_target, float l_decay, float l_spot) {
    // -> Direct
    vec4 directLightCol = l_color;
    // -> Point
    vec4 pointLightCol = l_color * pow(l_target / length(l_pos - fs_pos), l_decay);
    // -> Spot
    vec4 spotLightCol = pointLightCol * l_spot;
...
void main() {
    mat3 tbn = compTBN(fs_pos,fs_norm,fs_uv);
    ...
    // compute a single light
	vec3 dir_pos_l_fs = compLightDir(l_type, l_pos, l_dir);
	float Seen_l1 = clamp(dot(dir_pos_l_fs, norm),0.0,1.0);
    float Seen_l1_spot = spotIntensity(l_dir, dir_pos_l_fs, l_coneOut,l_coneIn );
    vec4 ol_color = compLightColor(l_type, l_pos, l_color, l_target, l_decay, Seen_l1_spot);

    vec4 Lambert_l1 = Seen_l1 * ol_color;
    float SpecShine = 2.0;
	vec4 Phong_l1 = pow(clamp(dot(c_dir, -reflect(dir_pos_l_fs,norm)),0.0,1.0), SpecShine) * ol_color * (Seen_l1==0.0 ? 0.0:1.0);


    // compute point light ball
	vec3 dir_pos_l2_fs = compLightDir(l_ball_type, l_ball_pos, l_ball_dir);
	float Seen_l2_main = max(min(dot(dir_pos_l2_fs, fs_norm),1.0),0.0);
	float Seen_l2 = clamp(dot(dir_pos_l2_fs, norm)*Seen_l2_main,0.0,1.0);
    float Seen_l2_spot = spotIntensity(l_ball_dir, dir_pos_l2_fs, l_ball_coneOut,l_ball_coneIn );
    vec4 ol2_color = compLightColor(l_ball_type, l_ball_pos, l_ball_color, l_ball_target, l_ball_decay, Seen_l2_spot);

    vec4 Lambert_l2 = Seen_l2 * ol2_color;
	vec4 Phong_l2 = pow(clamp(dot(c_dir, -reflect(dir_pos_l2_fs,norm)),0.0,1.0), SpecShine) * ol2_color * (Seen_l2==0.0 ? 0.0:1.0);


    out_color = albedo * Lambert_l1;
    out_color += albedo * Phong_l1;
    out_color += albedo * Lambert_l2;
    out_color += albedo * Phong_l2;
}
`;
setup = {
    init: function () {
        ...
        this.globalsLight = {
            l_type:[0,0,1],
            l_ambient: RgbTo01(100,100,100,1),
    ...
    lightsKeyDown(e){
		switch (e.code) {
			...
			case "KeyL":
				...
				break;
            case "KeyR":
                this.globalsLight.l_ball_pos[0] += e.key == e.key.toLowerCase() ? .1 : -.1;
                console.log(this.globalsLight.l_ball_pos);
                break;
            case "KeyT":
                this.globalsLight.l_ball_pos[1] += e.key == e.key.toLowerCase() ? .1 : -.1;
                console.log(this.globalsLight.l_ball_pos);
                break;
            case "KeyY":
                this.globalsLight.l_ball_pos[2] += e.key == e.key.toLowerCase() ? .1 : -.1;
                console.log(this.globalsLight.l_ball_pos);
                break;
			// w,h,n,f
			// case "KeyV":
