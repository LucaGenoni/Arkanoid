...
void main() {
    ...
    vec4 OlightColor = compLightColor(l_type, l_pos, l_color, l_target, l_decay, Seen_l1_spot);

    vec4 Lambert_l1 = Seen_l1 * OlightColor;
    float SpecShine = 1.0;
	vec4 Phong_l1 = pow(clamp(dot(c_dir, -reflect(dir_pos_l_fs,norm)),0.0,1.0), SpecShine) * l_color * (Seen_l1==0.0 ? 0.0:1.0);
	

    out_color = albedo * Lambert_l1 + vec4(1,1,1,1) * Phong_l1 + albedo;
}
`;
setup = {
