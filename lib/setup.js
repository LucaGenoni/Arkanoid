...
void main() {
	...
	vec3 dir_pos_l_fs = compLightDir(l_type, l_pos, dir);
	float Seen_l1 = clamp(dot(dir_pos_l_fs, norm),0.0,1.0);
    float Seen_l1_spot = spotIntensity(dir, dir_pos_l_fs, l_coneOut,l_coneIn );
	...
	vec3 dir_pos_l2_fs = compLightDir(l_ball_type, l_ball_pos, dir);
	float Seen_l2 = clamp(dot(dir_pos_l2_fs, norm),0.0,1.0);
    vec4 ol2_color = compLightColor(l_ball_type, l_ball_pos, l_ball_color, l_ball_target, l_ball_decay, Seen_l1_spot);
    ...
    out_color = albedo * (Lambert_l1 + Lambert_l2 + Phong_l1 + Phong_l2) ;
}
`;
setup = {
    ...
    _loadTextures: function () {
        ...
        this.textures = twgl.createTextures(gl, {
            ...
            sponde_norm : { crossOrigin: 'anonymous', src: "textures/sponde_norm.jpg" },
