...
void main() {
    ...
    if(l_type.z == 1.0) ol_color *= spotIntensity(l_dir, dir_pos_l_fs, l_coneOut,l_coneIn);
    ...
    // // reflection with normal map
    // vec4 ol2_color = compLightColor(l_ball_type, l_ball_pos, l_ball_color, l_ball_target, l_ball_decay);
    // // if light can reach ol2_color will not be black
    // ol2_color *= max(dot(dir_pos_l2_fs, fs_norm),0.0)==0.0 ? 0.0:1.0;
    // if(l_ball_type.z == 1.0) ol2_color *= spotIntensity(l_ball_dir, dir_pos_l2_fs, l_ball_coneOut,l_ball_coneIn);

    // float Reflection_l2 = clamp(dot(dir_pos_l2_fs, norm),0.0,1.0);
    // vec4 Lambert_l2 = ol2_color ;
	// vec4 Phong_l2 = pow(clamp(dot(c_dir, -reflect(dir_pos_l2_fs,norm)),0.0,1.0), SpecShine) * ol2_color * (Reflection_l2==0.0 ? 0.0:1.0);

	vec3 dir_pos_l2_fs = compLightDir(l_ball_type, l_ball_pos, l_ball_dir);
	float Reflection_pure_l2 = max(dot(dir_pos_l2_fs, fs_norm),0.0)==0.0 ? 0.0:1.0;
	float Reflection_l2 = clamp(dot(dir_pos_l2_fs, norm),0.0,1.0);
    float Reflection_l2_spot = spotIntensity(l_ball_dir, dir_pos_l2_fs, l_ball_coneOut,l_ball_coneIn );
    vec4 ol2_color = compLightColor(l_ball_type, l_ball_pos, l_ball_color, l_ball_target, l_ball_decay);
    if( l_ball_type.z == 1.0) ol2_color *= spotIntensity(l_ball_dir, dir_pos_l2_fs, l_ball_coneOut,l_ball_coneIn);

    vec4 Lambert_l2 = Reflection_l2 * ol2_color * Reflection_pure_l2;
	vec4 Phong_l2 = pow(clamp(dot(c_dir, -reflect(dir_pos_l2_fs,norm)),0.0,1.0), SpecShine) * ol2_color * (Reflection_l2==0.0 ? 0.0:1.0);
